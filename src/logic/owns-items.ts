...
const createQuery = (address: string, collectionIds: string[], itemIds: string[], timestamp: number) => `
...
export async function ownsItems(components: Pick<AppComponents, 'database'>, address: string, itemUrns: BlockchainCollectionV2Asset[], atTimestamp: number) {
  ...
  return itemUrns.filter(urn => ownedItemIds.has(`${urn.contractAddress}-${urn.id}`)).map(urn => `urn:${urn.namespace}:${urn.network}:collections-v2:${urn.contractAddress}:${urn.id}`)
}